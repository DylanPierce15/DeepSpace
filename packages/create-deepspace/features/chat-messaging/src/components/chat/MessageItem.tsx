...
import { useUserLookup, useUser } from 'deepspace'
import type { RecordData, Message, GroupedReaction } from 'deepspace'
  ...
  return (
    ...
    >
      ...
      {!isEditing && (
        <div
          data-hover-toolbar
          className="absolute -top-3 right-4 items-center gap-0.5 max-w-[280px] overflow-x-auto bg-card border border-border rounded-md shadow-sm p-0.5 z-10 transition-opacity opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto"
        ...
        >
          ...
          {EMOJI_LIST.map((emoji) => (
            <button
              ...
              className="shrink-0 w-7 h-7 rounded flex items-center justify-center text-sm hover:bg-muted transition-colors"
            >
              {emoji}
            </button>
          ))}
