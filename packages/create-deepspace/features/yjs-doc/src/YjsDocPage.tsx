 ...
 * useYjsField + Y.Text directly, instead of useYjsText which does
 * full-text replacement on every change.
 *
 * Key difference: on each input event we compute the minimal diff
 * (what was inserted/deleted at what position) and apply only those
 * operations to Y.Text. This means concurrent edits from multiple
 * users merge correctly without overwriting each other's changes.
 */
...
function CollabEditor({ documentId, title }: CollabEditorProps) {
  const { doc, synced, canWrite, updateCount } = useYjsField('documents', documentId, 'content')
  ...
  // Observe remote changes on Y.Text and update local state
  useEffect(() => {
    ...
    const observer = () => {
      ...
      const cursorBefore = ta?.selectionStart ?? 0
      setLocalText(value)
      // Update prevTextRef after React renders the new value,
      // so handleInput always diffs against what's actually in the textarea
      requestAnimationFrame(() => {
        prevTextRef.current = value
        if (ta && document.activeElement === ta) {
          ta.selectionStart = cursorBefore
          ta.selectionEnd = cursorBefore
        }
      })
    }
  ...
  // Sync initial text once synced
  useEffect(() => {
    if (synced && yTextRef.current) {
...
export default function YjsDocPage() {
  const { user } = useUser()
