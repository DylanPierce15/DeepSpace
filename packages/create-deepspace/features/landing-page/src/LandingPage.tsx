 ...
 * glassmorphic content overlay. The shell always renders Hero, CTA, and
 * Footer. Additional sections are imported and placed in the JSX stack
 * between Hero and CTA.
 *
 * All section components are pre-installed in components/landing/:
 *   FeaturesGridSection, ShowcaseSection, HowItWorksSection,
 *   TestimonialsSection, FAQSection, StatsSection, TeamSection,
 *   LogoCloudSection, VideoSection
 *
 * To use a section: import it here and place <SectionComponent /> in the
 * section stack. Delete the files for sections you don't need.
 */
...
// ============================================================================
// Landing page "seen" flag — localStorage helpers
// ============================================================================

const LANDING_SEEN_KEY = 'app-landing-seen'

/** Returns true if the user has previously visited the landing page. */
export function hasSeenLanding(): boolean {
  try { return localStorage.getItem(LANDING_SEEN_KEY) === 'true' } catch { return false }
}

/** Marks the landing page as seen (called when user clicks a CTA to enter the app). */
export function markLandingSeen(): void {
  try { localStorage.setItem(LANDING_SEEN_KEY, 'true') } catch {}
}

// ============================================================================
// Configuration — agent customizes these per app
// ============================================================================

/** Background image URL. Generate via mcapi or leave empty for a solid bg-background fallback. */
const LANDING_BG_URL = ''

const APP_NAME = 'My App'
const HERO_HEADLINE = 'Welcome to My App'
const HERO_SUBHEADLINE =
  'A simple, powerful way to get things done. Explore what\'s possible and start building today.'
const HERO_IMAGE = ''

/**
 * NAV_SECTIONS — single source of truth for scroll-tracked sections.
 *
 * Add/remove entries here when adding/removing landing page sections.
 * The `id` must match the `id` attribute on the corresponding <section> element.
 * Order doesn't matter — scroll highlighting is determined by DOM position.
 */
const NAV_SECTIONS = [
 ...
 * Sections are sorted by their actual DOM position on every tick,
 * so the order of `sectionIds` array doesn't matter.
 */
      ...
      >
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              className="md:hidden transition-colors text-muted-foreground hover:text-foreground"
      ...
      <AnimatePresence>
        {isScrolled && (
          ...
          >
            ...
            >
              <span className="text-foreground font-semibold text-sm px-3 whitespace-nowrap">
                {APP_NAME}
              </span>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'text-foreground bg-foreground/[0.12]'
                          : 'text-muted-foreground hover:text-foreground hover:bg-foreground/[0.08]',
                      )}
                    >
                      {link.label}
              ...
              <button
                className="md:hidden p-2 rounded-full text-muted-foreground hover:text-foreground hover:bg-foreground/[0.08] transition-colors"
                onClick={() => setMobileOpen(prev => !prev)}
                aria-label="Toggle menu"
              >
  ...
  return (
    <section ref={heroRef} className="relative">
      <div className="min-h-screen flex flex-col items-center justify-center px-6">
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          >
            <span
              className={cn(
                'inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-medium tracking-wide uppercase',
                'bg-foreground/[0.06] backdrop-blur-sm border border-foreground/[0.1]',
          ...
          <h1
            className={cn(
              'text-4xl sm:text-5xl md:text-7xl font-bold leading-[1.08] tracking-[-0.02em]',
              typewriterDone
                ? 'bg-gradient-to-r from-foreground via-foreground/80 via-50% to-foreground bg-clip-text text-transparent landing-shimmer-text'
                : 'text-foreground',
            )}
          >
            <Typewriter
          ...
          >
            ...
            <button
              ...
              className={cn(
                'inline-flex items-center gap-2 min-w-[180px] justify-center',
                'px-6 py-3 rounded-md text-sm font-medium',
                'border border-foreground/20 text-foreground bg-foreground/[0.06] hover:bg-foreground/[0.12]',
      ...
      <motion.div
        ...
        animate={typewriterDone ? { opacity: 1, y: 0 } : { opacity: 0, y: 60 }}
        transition={{ duration: 1, delay: 0.3, ease: [0.25, 0.4, 0.25, 1] }}
        style={{ y: mockupY }}
        className="relative z-10 w-full max-w-4xl mx-auto px-6 pb-32"
      >
...
// ============================================================================
// Showcase — alternating left/right blocks (default, removable)
// ============================================================================
...
// ============================================================================
// FAQ Accordion (default, removable)
// ============================================================================
    ...
    <section id="faq" className="relative py-28 md:py-36">
      <div className="max-w-3xl mx-auto px-6">
        <ScrollReveal className="text-center mb-16">
          ...
          <p className="mt-5 text-muted-foreground max-w-lg mx-auto text-lg leading-relaxed">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-10 md:gap-8">
          <div className="col-span-2">
            ...
            <p className="mt-3 text-sm text-muted-foreground/60 leading-relaxed max-w-xs">
              {HERO_SUBHEADLINE}
            </p>
          ...
          {Object.entries(FOOTER_LINKS).map(([heading, links]) => (
            <div key={heading}>
              <h4 className="text-xs font-semibold tracking-[0.15em] uppercase text-muted-foreground/60 mb-4">
                {heading}
              </h4>
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-center">
          <a
            ...
            className={cn(
              ...
              'text-[11px] text-muted-foreground/50 hover:text-muted-foreground hover:bg-foreground/[0.06]',
              'transition-all duration-200',
            )}
          >
...
// ============================================================================
// Page Shell — Fixed background + scrolling content
// ============================================================================
...
export default function LandingPage() {
  ...
  return (
    <div className="relative h-screen overflow-hidden bg-background">
      {/* Fixed background — renders only when LANDING_BG_URL is set */}
      {LANDING_BG_URL && (
      ...
      {/* Scrollable content — agent adds/removes sections here */}
      <div ref={containerRef} className="scrollable-content-layer">
        ...
        <HeroSection />
        <ShowcaseSection />
        {/* Import and place more sections here from components/landing/ */}
        <FAQSection />
        <CTASection />
        <Footer />
      </div>
