...
import { useTeams } from '@/client/storage/hooks/useTeams'
import type { Team } from '@/client/storage/types'
import {
  analyzePermissions,
  type ResolvedPermission,
  type CollectionPermissionSummary,
  type ColumnDefinition,
  type CollectionSchema,
} from '@/server/schemas/registry'
import { Badge, type BadgeProps } from '@/ui'
import { ROLES, ROLE_CONFIG, type Role } from '@/shared/roles'
...
export default function PermissionsPage({ schemas, className }: PermissionsPageProps) {
  const { user } = useUser()
  const { teams } = useTeams()
  const currentRole = user?.role ?? ROLES.VIEWER
  const roleConfig = ROLE_CONFIG[currentRole as Role] ?? ROLE_CONFIG[ROLES.VIEWER]

  const analysis = useMemo(() => analyzePermissions(schemas), [schemas])
...
function PermissionBadge({ resolved }: { resolved: ResolvedPermission }) {
  ...
  return (
    ...
    >
      <Badge variant={isDefaultDeny ? 'secondary' : display.variant as any}>
        {display.label}
