...
import { analyzePermissions, type ResolvedPermission, type CollectionPermissionSummary } from 'deepspace/worker'
import type { ColumnDefinition } from 'deepspace/worker'
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant={roleConfig.badgeVariant as BadgeVariant}>{roleConfig.title}</Badge>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bg-muted/40 rounded-xl border border-border p-4">
      ...
      <div className="flex flex-wrap gap-3 text-xs">
        <div className="flex items-center gap-1.5"><Badge variant="success">all</Badge><span className="text-muted-foreground">Everyone with role</span></div>
        <div className="flex items-center gap-1.5"><Badge variant="destructive">none</Badge><span className="text-muted-foreground">Denied</span></div>
        <div className="flex items-center gap-1.5"><Badge variant="default">own</Badge><span className="text-muted-foreground">Record owner only</span></div>
        <div className="flex items-center gap-1.5"><Badge variant="info">unclaimed/own</Badge><span className="text-muted-foreground">Unclaimed or owner</span></div>
        <div className="flex items-center gap-1.5"><Badge variant="warning">collaborator</Badge><span className="text-muted-foreground">Owner or collaborator</span></div>
      </div>
      <div className="flex gap-4 mt-3 pt-3 border-t border-border/30 text-xs text-muted-foreground">
        <span><span className="border-b border-dashed border-muted-foreground">dashed border</span> = resolved via * wildcard</span>
        <span><span className="italic opacity-50">italic + muted</span> = no permission defined (default deny)</span>
      </div>
    </div>
  )
}

function CollectionCard({ collection, roles, currentRole }: {
    ...
    <div className="bg-muted/40 rounded-xl border border-border overflow-hidden">
              ...
              return (
                <tr key={role} className={`border-b border-border/20 last:border-0 ${isCurrentRole ? 'bg-primary/10' : ''}`}>
                  <td className="px-4 py-2.5">
                    ...
                    {rc && <span className="text-[10px] text-muted-foreground ml-1.5">{rc.description}</span>}
                    {isCurrentRole && <span className="ml-2"><Badge variant="default">You</Badge></span>}
                  </td>
                  <td className="px-4 py-2.5 text-center"><PermissionBadge resolved={perms.read} /></td>
                  <td className="px-4 py-2.5 text-center"><PermissionBadge resolved={perms.create} /></td>
                  <td className="px-4 py-2.5 text-center">
                    <div>
                      <PermissionBadge resolved={perms.update} />
                      {perms.writableFields && (
                        <div className="mt-1 text-[10px] text-muted-foreground cursor-help" title={`Writable: ${perms.writableFields.join(', ')}`}>
                          {perms.writableFields.length} field{perms.writableFields.length !== 1 ? 's' : ''}
                        </div>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-2.5 text-center"><PermissionBadge resolved={perms.delete} /></td>
                </tr>
              )
            })}
          </tbody>
      ...
      {expanded && (
        <div className="border-t border-border/50">
          <ColumnDetailsTable columns={collection.columns} />
        </div>
      )}
    </div>
  )
...
function PermissionBadge({ resolved }: { resolved: ResolvedPermission }) {
  ...
  return (
    <span title={title} className={`inline-flex ${isDefaultDeny ? 'opacity-50 italic' : ''} ${isWildcard ? '[&>div]:border-dashed' : ''}`}>
...
function ColumnDetailsTable({ columns }: { columns: ColumnDefinition[] }) {
  if (columns.length === 0) {
        ...
        <tbody>
          {columns.map(col => (
            <tr key={col.name} className="border-b border-border/10 last:border-0">
              <td className="px-4 py-1.5"><code className="text-foreground font-medium">{col.name}</code></td>
              <td className="px-4 py-1.5 text-muted-foreground">{col.storage}</td>
              <td className="px-4 py-1.5 text-muted-foreground">{typeof col.interpretation === 'string' ? col.interpretation : JSON.stringify(col.interpretation)}</td>
              <td className="px-4 py-1.5 text-center">{col.required && <CheckIcon />}</td>
              <td className="px-4 py-1.5 text-center">{col.immutable && <CheckIcon />}</td>
              <td className="px-4 py-1.5 text-center">{col.userBound && <CheckIcon />}</td>
            </tr>
          ))}
        </tbody>
      </table>
